...
sum(results.lrt$padj < 0.05 & results.lrt$log2FoldChange > 0, na.rm = T) # 34 upregulated miRNAs
sum(results.lrt$padj < 0.05 & results.lrt$log2FoldChange < 0, na.rm = T) # 101 upregulated miRNAs

## Comparing Cachectic vs. Non-cachectic (in Kidney model)
KC_vs_KNC = results(dds, contrast=c("condition", "KC", "KNC"), independentFiltering = TRUE, alpha=0.05, test="Wald")
KC_vs_KNC <- lfcShrink(dds=dds, contrast=c("condition", "KC", "KNC"), type="ashr") #ratio: KC/KNC
hist(KC_vs_KNC$pvalue, xlab="pvalue", main="KC vs KNC") 
sum(KC_vs_KNC$padj < 0.05 & KC_vs_KNC$log2FoldChange > 0, na.rm = T) # ?? WHY IS IT 101 ??
...
sum(de_mirna$log2FoldChange > 0) #34 upregulated
...
sum(significant_results$log2FoldChange > 0) # 34 upregulated
sum(significant_results$log2FoldChange < 0) # 101 downregulated
...
length(unique(de_mirna_targets_filtered$miRNA[de_mirna_targets_filtered$log2FoldChange > 0])) # 12 upregulated miRNAs of 22 
length(unique(de_mirna_targets_filtered$miRNA[de_mirna_targets_filtered$log2FoldChange < 0])) # 40 downregulated miRNAs of 101
...
sum(unique(de_mirna_targets_filtered$Gene.Symbol[de_mirna_targets_filtered$log2FoldChange > 0]) %in% microarray_genes$Symbol[microarray_genes$logFC < 0]) # 177 downregulated genes targetted by 12 upregulated miRNAs (out of 340 gene targets)
...
length(unique(de_mirna_up_targets_filtered$Gene.Symbol)) # 177

sum(unique(de_mirna_targets_filtered$Gene.Symbol[de_mirna_targets_filtered$log2FoldChange < 0]) %in% microarray_genes$Symbol[microarray_genes$logFC > 0]) # 328 upregulated genes targetted by 40 downregulated miRNAs (out of 719 gene targets)
...
length(unique(de_mirna_down_targets_filtered$Gene.Symbol)) # 328
...
library(miRBaseConverter)
library(gprofiler2) # ID converter
library(biomaRt)
...
length(unique(up_results_ensembl)) # 177
...
length(unique(down_results_ensembl)) # 329
length(unique(de_mirna_down_targets_filtered$Gene.Symbol)) #328
...
#GO ontology: does hypergeometric test -- Gene targets of UPREGULATED miRNAs
 ...
 ego_up_interesting_desc = c("skeletal muscle cell proliferation", "muscle organ development", "regulation of skeletal muscle cell proliferation", "muscle structure development", "collagen fibril organization", "regulation of fibroblast proliferation", "myoblast proliferation", "cartilage development involved in endochondral bone morphogenesis")
...
table(interesting_genes_up$miRNA) # 9 miRNAs: hsa-miR-100-5p hsa-miR-149-5p hsa-miR-150-5p hsa-miR-204-5p hsa-miR-222-3p hsa-miR-29b-3p hsa-miR-328-3p hsa-miR-92a-3p hsa-miR-92b-3p 
...
boxplot(interesting_genes_up$Weighted_Context_Score ~ interesting_genes_up$miRNA,
        las = 2,
        ylab = "Weighted Context Score of Gene Target",
        xlab = "miRNA",
        main = "Distribution of Weighted Context Score per miRNA")
#Distribution of LFC of expression of gene targets of above miRNAs
boxplot(interesting_genes_up$micro_LFC ~ interesting_genes_up$miRNA,
        las = 2,
        ylab = "Log Fold Change of Target Genes Expression",
        xlab = "miRNA",
        main = "Distribution of LFC of Gene Targets per miRNA")
plot(interesting_genes_up$Weighted_Context_Score,
     interesting_genes_up$micro_LFC,
     ylim = c(-2,0),
     xlim = c(-0.7,0),
     ylab = "Log Fold Change of Target Gene Expression",
     xlab = "Weighted Context Score of Target Genes")
...
# table(interesting_genes_up_filtered$miRNA) #3 miRNAs: hsa-miR-149-5p, hsa-miR-204-5p, hsa-miR-29b-3p 
...
#GO ontology: does hypergeometric test -- Gene targets of DOWNREGULATED miRNAs
 ...
 head(ego_down) # NOTE: down are not enriched for anything
  
dotplot(ego_up,
        title = "Gene Targets of Upregulated miRNAs")
