...
```{r}
microarray_genes = read.csv("de_genes_microarray.csv")

de_mirna_targets_filtered = de_mirna_targets[de_mirna_targets$Gene.Symbol %in% microarray_genes$Symbol,]
nrow(de_mirna_targets_filtered) # 3070

length(unique(de_mirna_targets_filtered$miRNA)) # 52 TOTAL miRNAs 
length(unique(de_mirna_targets_filtered$miRNA[de_mirna_targets_filtered$log2FoldChange > 0])) # 12 upregulated miRNAs
length(unique(de_mirna_targets_filtered$miRNA[de_mirna_targets_filtered$log2FoldChange < 0])) # 40 upregulated miRNAs
...
## Note: overlap in gene targets?

table(table(de_mirna_targets_filtered$miRNA))

de_mirna_up_targets_filtered = de_mirna_targets_filtered[de_mirna_targets_filtered$log2FoldChange > 0,] #635
de_mirna_down_targets_filtered = de_mirna_targets_filtered[de_mirna_targets_filtered$log2FoldChange < 0,] #2435
...
hsa.gff.db <- makeTxDbFromGFF( "hsa.gff3", format="gff3" )
ensembl.genes = genes(hsa.gff.db)
#saveRDS(ensembl.genes, "mm.ensembl.genes.rds")
# ensembl.genes <- readRDS("mm.ensembl.genes.rds")
human = useEnsembl(biomart="ENSEMBL_MART_ENSEMBL",host="https://www.ensembl.org", dataset="hsapiens_gene_ensembl", version=108)
# mouse = useEnsembl(biomart="ENSEMBL_MART_ENSEMBL",mirror="asia", dataset="hsapiens_gene_ensembl", version="108")
...
ensembl.genes$hgnc_symbol = bm.annotations$mgi_symbol[ match(ensembl.genes$gene_id, bm.annotations$ensembl_gene_id) ]
...
ensembl.genes$entrezgene = bm.annotations$entrezgene_id[ match(ensembl.genes$gene_id, bm.annotations$ensembl_gene_id) ]

```



#### Clustering & Visualization & Enrichment Analysis - UPregulated miRNAs

```{r}


up_filtered_results = de_mirna_up_targets_filtered[,c(1:8)] 
rld_signif_up = assay(rld)[up_filtered_results$accession,]
...
set.seed(123)
k.max <- 10 # number of replicates/samples (N = k)
wss <- sapply(2:k.max, 
...
# custer function
go_up_cluster_function <- function(num){
  c1 = de_mirna_up_targets_filtered$Gene.ID[de_mirna_up_targets_filtered$accession %in% names(results.coef.kmeans$cluster[results.coef.kmeans$cluster == num])] 
  for (i in seq(length(c1))){
    c1[i] = substring(c1[i],1,15)
  }
...
# CLUSTER 1
ego_up_1 = go_up_cluster_function(1)
dotplot(ego_up_1)
ego.up.df.1 = as.data.frame(ego_up_1)

# CLUSTER 2
ego_up_2 = go_up_cluster_function(2)
dotplot(ego_up_2)
ego.up.df.2 = as.data.frame(ego_up_2)

# CLUSTER 3
ego_up_3 = go_up_cluster_function(3) # promising !!!
dotplot(ego_up_3)
ego.up.df.3 = as.data.frame(ego_up_3)

# CLUSTER 4
ego_up_4 = go_up_cluster_function(4) # promising !!!
dotplot(ego_up_4)
ego.up.df.4 = as.data.frame(ego_up_4)

# CLUSTER 5
ego_up_5 = go_up_cluster_function(5) # promising !!!
dotplot(ego_up_5)
ego.up.df.5 = as.data.frame(ego_up_5)


interesting_clusters_up = de_mirna_up_targets_filtered[de_mirna_up_targets_filtered$accession %in% unique(names(results.coef.kmeans$cluster[results.coef.kmeans$cluster %in% c(3:5)])),] 
# cluster 3: 3 miRNAs, cluster 4: 2 miRNAs, cluster 5: 4 miRNAs
length(unique(interesting_clusters_up$Gene.Symbol)) #284 gene targets
length(unique(interesting_clusters_up$miRNA)) # 9 miRNAs

```

#### Clustering & Visualization & Enrichment Analysis - DOWNregulated miRNAs

```{r}

down_filtered_results = de_mirna_down_targets_filtered[,c(1:8)] 
rld_signif_down = assay(rld)[down_filtered_results$accession,]
rld_z = t(apply(rld_signif_down, 1, function(x){ (x - mean(x)) / sd(x)})) # converting to zscore --> to scale
...
# custer function
go_down_cluster_function <- function(num){
  c1 = de_mirna_down_targets_filtered$Gene.ID[de_mirna_down_targets_filtered$accession %in% names(results.coef.kmeans$cluster[results.coef.kmeans$cluster == num])] 
  for (i in seq(length(c1))){
    c1[i] = substring(c1[i],1,15)
  }
...
# CLUSTER 1
ego_down_1 = go_down_cluster_function(1)
dotplot(ego_down_1)
ego.down.df.1 = as.data.frame(ego_down_1)

# CLUSTER 2
ego_down_2 = go_down_cluster_function(2)
dotplot(ego_down_2)
ego.down.df.2 = as.data.frame(ego_down_2)

# CLUSTER 3
ego_down_3 = go_down_cluster_function(3)
dotplot(ego_down_3)
ego.down.df.3 = as.data.frame(ego_down_3)

# CLUSTER 4
ego_down_4 = go_down_cluster_function(4) # promising?
dotplot(ego_down_4)
ego.down.df.4 = as.data.frame(ego_down_4)




interesting_clusters_down = de_mirna_down_targets_filtered[de_mirna_down_targets_filtered$accession %in% unique(names(results.coef.kmeans$cluster[results.coef.kmeans$cluster %in% c(4)])),] 
length(unique(interesting_clusters_down$Gene.Symbol)) #332 gene targets
length(unique(interesting_clusters_down$miRNA)) # 11 miRNAs
